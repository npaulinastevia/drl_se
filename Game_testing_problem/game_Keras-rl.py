...
def kr_learn(algo='ddpg'):
        ...
        if algo=='ddpg':
            ...
            ddpg = DDPGAgent(nb_actions=nb_actions, memory=memory, gamma=0.99, actor=actor, critic=critic,
                                critic_action_input=action_input, nb_steps_warmup_critic=100, nb_steps_warmup_actor=10)
            ddpg.compile(Adam(lr=.00025), metrics=['mae']
                            )
        ...
        for i in range(10):
            ...
            buglist = []
            numberOfbugs = 0
            path = 'D:\drl_wuji\DDPGKR' + str(i+1)
